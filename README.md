...
# Данные
![image](https://user-images.githubusercontent.com/63803498/200648591-dbec95ed-80c4-4809-9ac1-88c732e0b3e5.png)
...
- `access_level` может принимать значение 1 или 2
    ...
    - вне зависимости от уровня доступа участники приглашённой группы имеют право оставлять комментарии под ответами (если они не были заблокированы)
### List
...
- Староста академической группы (Group Admin):
    ...
    - Закрыть группу
- Пользователь (User)
    - (Если состоит в группе, приглашённой в список, и не заблокирован группой-владельцем списка) редактировать ответ
...
[Оглавление](#навигация)
## 12. Закрытие группы
В случае, когда академическая группа завершает обучение, староста закрывает группу. В таком случае все публичные списки, которыми владеет группа, остаются доступными посетителям сайта. Но они никогда не смогут уже быть отредактированными.
